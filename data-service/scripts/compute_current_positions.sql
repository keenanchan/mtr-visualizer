...
JOIN (select line_code, line_name, direction, kmph from line_shapes) ls
  ON (r.line_code, r.direction) = (ls.line_code, ls.direction)
JOIN (select line_code, station_code, direction, station_name from station_metadata) d
  ON (r.line_code, r.destination, r.direction) = (d.line_code, d.station_code, d.direction)
